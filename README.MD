...
#https://github.com/maiconpl/AutoChemDescriptors/blob/main/AutoChemDescriptors_logo_05_02_26_from_powerpoint.png

<img src="./maiconpl/AutoChemDescriptors/blob/main/AutoChemDescriptors_logo_05_02_26_from_powerpoint.png" alt="Software Logo" width="100"/>

<img src="https://github.com/maiconpl/AutoChemDescriptors/blob/main/AutoChemDescriptors_logo_05_02_26_from_powerpoint.png" alt="Software Logo" width="100"/>
...
## Before you begin
1. Set up a Python 3 environment and install the packages listed in `requirements.txt` (RDKit, PySCF, DScribe, ASE, NumPy, SciPy, Matplotlib, scikit-learn, Torch, PyBERNY, Qiskit, qiskit_machine_learning).
...
- edit ~/.bashrc and insert: " export "PYTHONPATH=/mnt/d/workspace/AutoChemDescriptors.0.0.10:$PYTHONPATH" "
- source ~/.bashrch
...
def run_descriptor():
    ...
    input_flow_controller = {
        ...
        "descriptors_type": "SMILES",    # mandatory: {"RDKIT","MBTR"}
    }

    molecules_coded_list = ["COc1ccccc1O"]  # mandatory: list of encodings, in this calse SMILES. It is also possible to use SELFIES.
...
- **`analysis` (fully optional):** Remove the dictionary entirely for headless descriptor generation or include only the subsections you need.
  ...
  - `kpca`; necessary to use classical kernel PCA (kpca) grouping, such as RBF or linear kernel.
  - `qkpca`; necessary to use quantum kernel PCA (qkpca) grouping, Qiskit and qiskit_machine_learning libraries are required.
  ...
  - `pcapg` activates the PCA + Possibilistic Graph (PCAPG) selector. Provide `n_components` (latent dimension), `alpha` (graph-preservation weight), `beta` (L2,1 sparsity strength), `lambda_reg` (graph regularization), `n_neighbors` (adaptive graph degree), and `top_features` (how many descriptors appear in the ranking/plots). Optional keys mirror other analyses: `scaling` (defaults to `"standard"`), `max_iter`, `tol`, and filename overrides (`ranking_plot_filename`, `manifold_plot_filename`, `biplot_plot_filename`, `convergence_plot_filename`, `csv_filename`). Running PCAPG automatically generates `pcapg_feature_scores.csv`, `plot_pcapg_importance.png`, `plot_pcapg_manifold.png`, `plot_pcapg_biplot.png`, `plot_pcapg_convergence.png`, and the Markdown narrative `report_pcapg.md`, each styled for publication use. Biplot support is enabled by default; just ensure `top_features` is large enough to expose the descriptors of interest.
  - `pcapg_report` (optional) can override the Markdown filename (`report_filename`) and how many descriptors appear in the table (`top_descriptors`). The report is emitted automatically whenever `pcapg` is enabled, so no extra flag is required.
  - `shap_validation` activates the SHAP-based explainability workflow. Supply the vector of `targets` (one value per molecule) plus optional knobs such as `task` (`"regression"`/`"classification"`), `model` (`"random_forest"`, `"extra_trees"`, or `"hist_gradient_boosting"`), `n_estimators`, `scaling`, and visualization limits like `top_features`. Advanced options—including `background_size`, `dependence_plots`, and filename overrides for each PNG (`importance_plot_filename`, `beeswarm_plot_filename`, `dependence_plot_prefix`)—live inside the same dictionary so you never touch the global analysis config.
...
Inside the directory that contains `autochemdesc.py`, simply run the script—each example now inserts the repository root into `sys.path` before importing. In other words, `python autochemdesc.py` “just works,” even if you launch it from a nested project folder.

Prefer an explicit `PYTHONPATH`? Point it to the repository root (two levels up from the example) instead of the nested `auto_chem_descriptors/` folder; this mirrors the structure the runtime injector creates automatically. Example session (path names are illustrative):

```
(venv) user@labhost ~/chem_runs/run001 $ PYTHONPATH="../.." python autochemdesc.py
```
...
- **PCAPG outputs** (`pcapg_feature_scores.csv`, `plot_pcapg_importance.png`, `plot_pcapg_manifold.png`, `plot_pcapg_biplot.png`, `plot_pcapg_convergence.png`, `report_pcapg.md`) document the PCA + possibilistic-graph selector. The Markdown ties each figure to automated insights: the manifold highlights adaptive-typicality anchors/outliers, the projected loading biplot connects descriptors to clusters with axis-alignment tables, convergence diagnostics certify numerical stability, and the ranking narrative pinpoints which variables retain most of the projection energy so chemists know where to focus.
- **SHAP outputs** (`plot_shap_importance.png`, `plot_shap_beeswarm.png`, `plot_shap_dependence.png`, `report_shap_validation.md`) quantify descriptor influence on the trained surrogate via mean-absolute SHAP rankings, swarm distributions across the library, and a multi-panel dependence figure colored by the target property. The Markdown report consolidates those visuals with model metrics, target statistics, cut-ready ranking tables, dependence correlations, and local sample narratives so the explainability story travels as a single publication-ready artifact.
...
- `analysis["dbscan_report"]` (optional dict) lets you rename the Markdown file (`report_filename`, default `dbscan_report.md`, and any custom value is auto-normalized so it begins with `dbscan_`) and cap how many entries appear in the noise/border/tail tables (`noise_limit`, `border_limit`, `sparse_limit`). The report always includes the density interpreter (PCA × K-Means × DBSCAN narrative, epsilon discrepancy, chemoinformatics notes) plus a generic decision playbook so each run documents both specific findings and reusable guidance.
...
## Configuring SHAP validation
- Enable `analysis["shap_validation"]` to obtain publication-grade explainability for any descriptor matrix. You must provide one target value per molecule (order matters and must mirror `molecules_coded_list`).
- `task` controls the surrogate objective (`"regression"` or `"classification"`). `model` accepts `"random_forest"`, `"extra_trees"`, or `"hist_gradient_boosting"`. Tune the usual scikit-learn knobs (`n_estimators`, `max_depth`, `min_samples_leaf`, etc.) if you need a specific bias/variance profile.
- Optional flags: `scaling` (`"none"` or `"standard"`), `top_features` (how many descriptors appear in the global ranking and downstream plots), `background_size` (TreeSHAP reference set), `dependence_plots` (how many descriptors to showcase in the multi-panel dependence figure), and filename overrides (`importance_plot_filename`, `beeswarm_plot_filename`, `dependence_plot_filename`).
- Outputs: `plot_shap_importance.png` (global ranking), `plot_shap_beeswarm.png` (distribution across molecules), and `plot_shap_dependence.png` (grid of Arrhenius-style dependence panels) plus console metrics (R²/RMSE or accuracy/AUC). All figures share the same encoding so you can drop them into a manuscript without extra editing.
- `analysis["shap_validation_report"]` (optional dict) lets you rename `report_shap_validation.md`, trim how many descriptors/samples appear in the Markdown, or override local-example limits. The report stitches together performance metrics, target quartiles, the global mean-|SHAP| table, dependence correlations, embedded figure references, and narrative bullet points so downstream reviewers consume the full SHAP context without opening notebooks.

### SHAP configuration example
The snippet below mirrors the Patricia project but trimmed to the seven SMILES shown earlier in this README. Replace the target values with your experimental or simulated measurements while preserving the order.
...
analysis = {
    ...
    "shap_validation": {
        "targets": [
            0.88,  # COc1ccccc1O
            0.90,  # COc1ccccc1OCC(=O)O
            0.92,  # COc1ccccc1OCC(=O)Nc1ccccc1
            0.91,  # COc1ccccc1OCC(=O)Nc1ccccc1F
            0.89,  # COc1ccccc1OCC(=O)Nc1cccc(F)c1
            0.90,  # COc1ccccc1OCC(=O)Nc1ccc(F)cc1
            0.93   # COc1ccccc1OCC(=O)Nc1cccc2ccccc12
        ],
        "task": "regression",
        "model": "random_forest",
        "top_features": 12,
...
Best practices:
- **Targets:** numeric vectors for regression or integer labels for classification; length must equal the number of molecules and the order must be identical. Comments help future readers map values back to SMILES.
- **Surrogate fidelity:** keep `background_size` between 100 and 500 to balance TreeSHAP stability and runtime. Use `kernel_nsamples` only when forcing KernelSHAP.
- **Dependence panels:** `dependence_plots` picks the top-N descriptors (based on mean |SHAP|) to visualize in `plot_shap_dependence.png`. All subplots share the same colormap and legend, making the figure manuscript-ready.
